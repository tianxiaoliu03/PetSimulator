...
food_image = pygame.transform.scale(food_image, (100, 100))  # Double the size
...
water_image = pygame.transform.scale(water_image, (100, 100))  # Double the size
water_rect = water_image.get_rect(topleft=(120, 540))
...
def draw_health_bar(screen, x, y, value, label):
    ...
    if value <= 0:
        value = 0
        color = RED
        if all(val <= 0 for val in [pet.hunger, pet.sleepiness, pet.happiness, pet.thirstiness]):
            message = "YOUR PET HAS DIED."
        else:
            message = ""

        # Render message text if message is not empty
        if message:
            ...
            message_rect.left - 5, message_rect.top - 5, message_rect.width + 10, message_rect.height + 10),
                             border_radius=20)
            pygame.draw.rect(screen, color, (
            message_rect.left - 5, message_rect.top - 5, message_rect.width + 10, message_rect.height + 10), width=2,
                             border_radius=20)
            # Draw message text
...
while run:
        ...
        if event.type == pygame.QUIT:
            ...
            if event.button == 1:  # Left mouse button
                ...
                if coin.rect.collidepoint(event.pos):
                    ...
                    if pet.coins >= 5:  # Check if there are enough coins to buy food
                        dragging_food = True
                    else:
                        message_timer = current_time + 2000  # Display message for 2 seconds
                elif water_rect.collidepoint(event.pos):
                    if pet.coins >= 5:  # Check if there are enough coins to buy water
                        dragging_water = True
                    else:
                        message_timer = current_time + 2000  # Display message for 2 seconds
            ...
            if event.button == 1:  # Left mouse button
                ...
                if coin.rect.collidepoint(event.pos):
                    ...
                    if pet.coins >= 5:  # Check if there are enough coins to buy water
                        ...
                        message_timer = current_time + 2000  # Display message for 2 seconds
                elif buttons["play"].collidepoint(event.pos) and not intro_screen:
                    pet.happiness += 10  # Increase sleepiness when "Sleep" button is clicked
                elif buttons["sleep"].collidepoint(event.pos) and not intro_screen:
                    pet.sleepiness += 10  # Increase sleepiness when "Sleep" button is clicked
    ...
    if intro_screen:
        ...
        # Draw health bars
        draw_health_bar(screen, 10, 10, pet.hunger, "Satiated Hunger")
        draw_health_bar(screen, 10, 40, pet.sleepiness, "Awakeness")
        draw_health_bar(screen, 10, 70, pet.happiness, "Happiness")
        draw_health_bar(screen, 10, 100, pet.thirstiness, "Quenched Thirst ")

        coins_text = my_font.render(f"Coins: {pet.coins}", True, BLACK)
        screen.blit(coins_text, (10, 130))

        # Display "Not enough coins!" message if needed
        if message_timer != 0 and pet.coins < 5:
            message_text = my_font.render("Not enough coins!", True, RED)
            message_text_rect = message_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 4))
